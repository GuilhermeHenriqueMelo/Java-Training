...
import com.jsf.jpa.SchoolEntityManager;
...
public class DatabaseOperations {
	...
	// Method To Update The School Details For A Particular School Id In The
	// Database
	public static String updateSchoolDetails(int schoolId, String updatedSchoolName) {
		...
		if (isSchoolIdPresent(schoolId)) {
			...
			if (updateCount > 0) {
				...
				refreshUpdatedItem(schoolId);
			}
		}
		
		transactionObj.commit();
		FacesContext.getCurrentInstance().addMessage("editSchoolForm:schoolId",
				new FacesMessage("School Record #" + schoolId + " Is Successfully Updated In Db"));
	...
	// Helper Method 2 - Fetch Particular School Details On The Basis Of School Id
	// From The Database
	...
	private static void refreshUpdatedItem(int id) {
		Query query = entityMgrObj.createQuery("SELECT s FROM SchoolEntityManager s WHERE s.id= :id");
		query.setParameter("id", id);
		
		Object o = query.getSingleResult();
		
		entityMgrObj.refresh(o);
	}
